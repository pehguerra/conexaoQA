    ...
    return (
        <div className="post bg-white p-1 my-1" data-test={`post-${_id}`}>
            ...
            <div>
                ...
                {showActions && 
                    <Fragment>
                        <button onClick={() => addLike(_id)} type="button" className="btn btn-light" data-test="post-like">
                            <i className="fas fa-thumbs-up"></i>
                            {likes.length > 0 && <span> {likes.length}</span>}
                        </button>
                        <button onClick={() => removeLike(_id)} type="button" className="btn btn-light" data-test="post-unlike">
                        ...
                        {!auth.loading && user === auth.user._id && (
                            <button onClick={() => deletePost(_id)} type="button" className="btn btn-danger" data-test="post-delete">
